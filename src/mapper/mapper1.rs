...
impl RawMapper for MMC1Mapper {
    fn access_main_bus(&mut self, addr: u16, value: u8, write: bool) -> u8 {
        if write {
            todo!("MMC1Mapper::access_main_bus write {addr:04X} = {value:02X}")
        }
    ...
    fn access_ppu_bus(&mut self, addr: u16, value: u8, write: bool) -> u8 {
        match addr {
            0x0000..=0x1FFF => {
                ...
                match self.chr_mode {
                    CHRMode::Switch8KiB => {
                        let base_addr = (self.chr_bank_0 & !1) as usize;
